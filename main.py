...
def run_experiment(algo_choice, size, num_runs=5):
  """
  Executes a multi-seed benchmark for a specific algorithm and dataset size.
  Calculates statistical benchmarks and logs results to results.txt.
  """
  seeds = [10, 20, 30, 40, 50]  # Fixed seeds for reproducibility
  algo_names = {
    '1': 'Hill Climbing', 
  ...
  for i in range(num_runs):
    ...
    # Reset RNG state for consistent results
    np.random.seed(seed)
    random.seed(seed)
    ...
    # Every run starts with a common baseline (Random Solution)
    ...
    if algo_choice == '1':
      ...
      final_path = initial_state # Random Baseline
  ...
  # STATISTICS: Compile and save results
  if results:
    ...
    # PERSISTENCE: Write results to permanent log file
    ...
    # VISUALIZATION: Save plot of the last successful run
    last_run = results[-1]
...
def run_menu():
  ...
  Handles user input for algorithm selection and dataset scaling.
  """
  while True:
    print("\n" + "=" * 40)
    print("  Traveling Santa Problem Solver  ")
    print("=" * 40)
    ...
    # Adaptive Menu: Hide slower options for Hill Climbing
    if algo_choice != '1':
      print("3. 1000 cities")
      if algo_choice == '3':
